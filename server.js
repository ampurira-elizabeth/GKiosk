...
var server = http.createServer(function (req, res) {   // create web server
  ...
  if (req.url == '/') { // check the URL of the current request
    ...
    res.write(JSON.stringify(greens.getItems()));
    res.end();
  }
  else if (req.url == "/products/fruits") {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.write(JSON.stringify(greens.getItems('fruits')));
    res.end();
  }
  else if (req.url == '/products/vegetables') {
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.write(JSON.stringify(greens.getItems('vegetables')));
    res.end();
  }
  else
